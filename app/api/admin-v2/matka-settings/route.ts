...
export async function POST(req: Request) {
  try {
    console.log("\n[ADMIN V2 MATKA SETTINGS SAVE] === START ===");
    await connectDB();
    ...
    console.log("[MATKA SETTINGS API] RECEIVED PAYLOAD:", body);

    const { _id, __v, createdAt, updatedAt, ...updateData } = body;

    const settings = await MatkaSettings.findOneAndUpdate(
      {},
      { $set: updateData },
      { new: true, upsert: true, runValidators: true }
    );

    console.log("[MATKA SETTINGS API] SAVED SUCCESSFULLY:", settings);
    return NextResponse.json({ success: true, settings });
  } catch (error: any) {
    console.error("[MATKA SETTINGS API] CRITICAL SAVE ERROR:", error.message);
...
export async function GET(req: Request) {
  try {
    ...
    console.log("[MATKA SETTINGS API] LOADED:", settings);
    return NextResponse.json({ success: true, settings });
  } catch (error: any) {
    console.error("[MATKA SETTINGS API] LOAD ERROR:", error.message);
